...
import com.mecury.okhttplibrary.internal.cache.CacheRequest;
...
public final class Cache implements Closeable, Flushable {
    ...
    private CacheRequest put(Response response) {
        ...
        if (!requestMethod.equals("GET")) {
            ...
            //不缓存非get请求的数据。从技术上讲，我们允许缓存请求头和一些POSt请求，但是做的多而收益少
            return null;
        }
     ...
     * 初始化cache，包括从内存中读取的分类文件 和 生成的在内存中的必要的信息。
     *
     ...
     * 初始化的时间依赖于分类的文件大小和目前实际的缓存大小。应用应该避免调用这个模块在初始化阶段，最好在后台进程中
     *
     ...
     * 关闭缓存并删除所有的values。它会删除cache中所有的文件
     */
            ...
            @Override public boolean hasNext() {
                ...
                while (delegate.hasNext()) {
                    ...
                    try {
                        ...
                        //我们不能读取主要程序从snapshot；可能因为主机文件系统已经消失，跳过他
                    } finally {
                        snapshot.close();
                    }
                }
    ...
    private static final class Entry {
         ...
         * HTTPS的responses包含SSL信息。它以一个空白行开头，然后一行包含密码格式。下一行是对等证书链的长度。
         * 这些证书是 base64-encoded， 然后显示在他们的一行。下一行包含本地证书链的长度，这些证书也是
         * base64-encoded，也显示在自己的一行。-1被用来编码一个空的数组。最后一行是可选的，如果存在，则包含TLS版本
         */
