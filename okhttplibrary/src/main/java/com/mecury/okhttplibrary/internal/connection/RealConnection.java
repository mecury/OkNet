...
import com.mecury.okhttplibrary.Handshake;
...
import com.mecury.okhttplibrary.internal.Version;
...
import com.mecury.okhttplibrary.internal.http2.Http2Connection;
import com.mecury.okhttplibrary.internal.http2.Http2Stream;
...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    /** The low-level TCP socket. 低等级的TCP socket*/
    ...
    public void connect(int connectTimeout, int readTimeout, int writeTimeout,
        ...
        if (protocol != null) throw new IllegalStateException("already connected");//协议不为null，表明进行过连接
    ...
    /**
     * Does all the work to build an HTTPS connection over a proxy tunnel(隧道连接). The catch here is that a
     * proxy server can issue an auth challenge and then close the connection.
     * 所有的工作建立在proxy tunnel 的HTTPS连接。这里的前提条件是，代理server可以发出一个权威性的挑战然后关闭连接
     */
     ...
     * 是否所有的工作有必要建立在一个完全的HTTP 或 HTTPS 连接在原始的socket
     */
    ...
    private void establishProtocol(int readTimeout, int writeTimeout,
        ...
        if (protocol == Protocol.HTTP_2) {
            socket.setSoTimeout(0); // Framed connection timeouts are set per-stream.设置连接超时判定
    ...
    private void connectTls(int readTimeout, int writeTimeout,
        ...
        try {
            // Create the wrapper over the connected socket.包装socket
     ...
     * 基于代理建立一个HTTPS的连接，发送一个加密的CONNECT请求来创建代理连接。如果代理需要授权，这可能需要重试
     */
