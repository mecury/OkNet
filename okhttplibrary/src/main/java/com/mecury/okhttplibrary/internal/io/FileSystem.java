 ...
 */
package com.mecury.okhttplibrary.internal.io;
...
/**
 * Access to read and write files on a hierarchical(分层) data store. Most callers should use the {@link
 ...
 * 在分层的数据存储中读和写。大部分调用者都应该使用SYSTEM这个实现，用来使用本地的文件系统。内用的实现能够用来注入
 * 错误的数据(为了测试)或移动这些存储的数据
 *
...
public interface FileSystem {
   ...
   * necessary.
   * 写入file，丢失所有已经展现的数据。有必要的话创建父目录创建
   */
